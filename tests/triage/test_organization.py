...
from email_agent.persistence import CategorySync, Message, Triage, initialize_database
from email_agent.providers.models import EmailMessage, EmailThread
from email_agent.triage.models import TriageOutput
from email_agent.triage.organization import OrganizationService, OrganizationStatus
...
def test_dry_run_retriage_has_no_side_effects(tmp_path):
    ...
    report = OrganizationService("person@example.com", account(), provider, Agents()).run(
        dry_run=True, retriage_all=True
    )
