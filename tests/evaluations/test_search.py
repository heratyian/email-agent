...
def test_search_target_exposes_stable_keys_from_production_graph_state():
    class FakeGraph:
    ...
    output = search_target(FakeGraph(), {7: "reply_request"})(
...
def test_empty_search_requires_no_retrieval_and_no_citations():
    ...
    assert empty_answer_accuracy(
        {"retrieved_keys": [], "cited_keys": []}, reference
    ) is True
    assert empty_answer_accuracy(
        {"retrieved_keys": ["unrelated"], "cited_keys": []}, reference
    ) is False


def test_checked_in_search_corpus_has_stable_unique_keys(tmp_path):
    from email_agent.db import initialize_database
    from email_agent.evaluations.classification import load_profile
...
def test_search_evaluation_uses_serial_blocking_production_graph(monkeypatch):
    client = FakeClient()
    graph = object()
    ...
    monkeypatch.setattr(
        "email_agent.evaluations.search.build_inbox_search_graph",
