...
def _migrate_triage_schema() -> None:
    ...
    if "triages" not in tables and "classifications" not in tables:
        return
    columns = {
        row[1] for row in database.execute_sql('PRAGMA table_info("triages")').fetchall()
    }
    if "category_sync_pending" not in columns:
        database.execute_sql(
            'ALTER TABLE "triages" ADD COLUMN "category_sync_pending" '
            "INTEGER NOT NULL DEFAULT 0"
        )

    indexes = {
        row[1] for row in database.execute_sql('PRAGMA index_list("triages")').fetchall()
    }
    if "classifications_message_id" in indexes:
        database.execute_sql('DROP INDEX "classifications_message_id"')
    database.execute_sql(
        'CREATE UNIQUE INDEX IF NOT EXISTS "triages_message_id" '
        'ON "triages" ("message_id")'
    )
