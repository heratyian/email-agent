...
from email_agent.triage.prompt import format_thread, triage_system_prompt
...
class EmailTriager:
    ...
    def __init__(self, root: Path, agent: AgentConfig, model):
        self.agent = agent
        self.system_prompt = redact(triage_system_prompt(root, agent)).sanitized_text
    ...
    def triage(self, message: EmailMessage, thread: EmailThread) -> TriageOutput:
        ...
        content = redact(format_thread(thread, message), known_names=known_names).sanitized_text
        logger.info(
        ...
        if triage.category is not None and triage.category not in self.agent.categories:
