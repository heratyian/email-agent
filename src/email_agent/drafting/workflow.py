...
class DraftService:
    ...
    def source_message(self, message_id: int, settings: Settings) -> EmailMessage:
        ...
        return provider.get_message(message_row.provider_uid, message_row.provider_mailbox)
    ...
    def generate(
        ...
        source = provider.get_message(message_row.provider_uid, message_row.provider_mailbox)
        thread = provider.get_thread(message_row.provider_uid, message_row.provider_mailbox)
        if instruction:
