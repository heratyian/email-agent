...
class AssistantConversation:
    ...
    def __init__(self, account_id: str, model, handlers):
        self.account_id = account_id
        self.planner = model.with_structured_output(AssistantIntentOutput)
        self.tools = make_assistant_tools(handlers)
    ...
    def _interpret(self, state: AssistantState) -> dict:
        ...
        if state.get("pending_action") and normalized in CONFIRMATIONS:
            return {"intent": AssistantIntentOutput(action="unsupported", explanation="confirm")}
        if state.get("pending_action") and normalized in CANCELLATIONS:
            return {"intent": AssistantIntentOutput(action="unsupported", explanation="cancel")}
        ...
        intent = AssistantIntentOutput.model_validate(
            self.planner.invoke(f"{ASSISTANT_PROMPT}\n\nSession context:\n{context}")
        )
    ...
    def _inbox(self, state: AssistantState) -> dict:
        result = self.tools["fetch_inbox"].invoke(
            {"account_id": state["account_id"], "limit": state["intent"].limit}
        )
        return {
            "turn": AssistantTurn(kind="inbox", payload=result.items),
            "last_message_ids": [item.local_id for item in result.items],
        }
